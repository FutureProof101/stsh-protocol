/**
 * J-25 — selection of the ONE release-record value the wallet build injects.
 *
 ...
 *
 * Why a hand-rolled section scanner rather than a TOML library: the wallet has
 * no TOML dependency and the gate installs from the committed lockfile only
 ...
 * make the emitted bundle depend on its own recorded digest, and the
...
/** The define name the wallet bundle carries. Referenced by vite + vitest. */
 ...
 *
 * EXPORTED (J-17b) so the `[wasm.*]` pin extractor in `wasmPins.ts` uses THIS
 * scanner rather than a second, divergent one. It deliberately does NOT strip
 * the quotes from `[wasm."stsh-verifier"]` — the consumer decides, and the
 * quoted section is the one a naive splitter gets wrong.
 */
export const SECTION_RE = /^\[\s*([^\]]+?)\s*\]\s*(?:#.*)?$/;
/** `key = "value"`, ignoring a trailing comment. Only double-quoted scalars. */
export const STRING_KV_RE = /^([A-Za-z0-9_-]+)\s*=\s*"([^"]*)"\s*(?:#.*)?$/;
...
export function selectBuildSourceSha(toml: string): string {
  ...
  if (found === null) {
    throw new ReleaseRecordError(
      "release record has no [build].source_sha; the wallet build source cannot be identified",
    );
  }
